...
class SaplCache {
    ...
    public static function ttlFor(string $path): int {
        ...
        if (str_contains($path, '/parlamentares/parlamentar')) return 168;
        if (str_contains($path, '/parlamentares/mandato'))     return 168;
        if (str_contains($path, '/parlamentares/frente'))      return 168;
        if (str_contains($path, '/parlamentares/frentecargo')) return 168;
        if (str_contains($path, '/parlamentares/filiacao'))    return 168;
        if (str_contains($path, '/base/autor'))                return 168;
        if (str_contains($path, '/comissoes/'))                return 168;
        if (str_contains($path, '/materia/relatoria'))         return 168;
        if (str_contains($path, '/materia/'))                  return 168;
        if (str_contains($path, '/norma/tiponorma'))           return 168;
        if (str_contains($path, '/norma/'))                    return 168;
        return 168;
    }
}
